...
load(paste0('sumlmHorizontal_type_',rangeStatList[2],'.Rdata'))
...
m = ggplot(sumlmHorizontal, aes(x=Beta)) + 
  ...
  geom_vline(xintercept=c(1,5/6,2/3,7/6),alpha=0.6, lwd=1.5,
...
dir.create(paste0("C:/Sync/CoolGirl/Fhe/Results/",modelname,"/clustering/"),showWarnings = F)
setwd(paste0("C:/Sync/CoolGirl/Fhe/Results/",modelname,"/clustering"))
file.remove(dir())
...
save(sumlmHorizontal, file=paste0(home, '/Results/',modelname,'/sumlmHorizontal_Districts_cluster.Rdata'))
write.csv(sumlmHorizontal, file=paste0(home, '/Results/',modelname,'/sumlmHorizontal_Districts_cluster.csv'))
