...
dir.create(paste0("C:/Sync/CoolGirl/Fhe/Results/",modelname,"/Balance/"),showWarnings = F)
setwd(paste0("C:/Sync/CoolGirl/Fhe/Results/",modelname,"/Balance/"))
file.remove(dir())
...
png(filename=paste0(home, '/Results/',modelname,'/Balance/balanceBeta.png'),
...
radarchart( dat  , axistype=1 , pty=32,
   ...
   vlcex=1, centerzero=T, caxislabels=seq(down, up, length.out=5),
   )
...
f0 = data.frame(year=c(1990,2000,2010))
s = unique(a$Group.1)
for (i in 1:4){
...
up = round(max(radardatt), 1) + 0.1
down = round(min(radardatt), 1) - 0.1
...
png(filename=paste0(home, '/Results/',modelname,'/Balance/balanceBeta1.png'),
		  width=15,height=15, units='cm',res=180)
...
AOVW = function(dat=suml, yearrange=c(1990,2000,2010), method='Greedy',sampling='weighted'){
	mean.w = c()
	SSTw = c()
	SSEw = c()
	SSAw = c()
	...
	aovdf = data.frame(
			...
			WB=c(rep('Inter-dimension',3),
				 rep('intra-dimension',3)),
...
### G + imbalance
av1 = AOVW(dat=suml, yearrange=c(1990,2000,2010), method='Greedy',sampling='unweighted')
av2 = AOVW(dat=suml, yearrange=c(1990,2000,2010), method='Greedy',sampling='weighted')
av3 = AOVW(dat=suml, yearrange=c(1990,2000,2010), method='Ideal',sampling='unweighted')
av4 = AOVW(dat=suml, yearrange=c(1990,2000,2010), method='Ideal',sampling='weighted')
...
png(filename=paste0(home, '/Results/',modelname,'/Balance/developmentIndex.png'),
...
png(filename=paste0(home, '/Results/',modelname,'/Balance/ImbalanceIndexCompare.png'),
		  width=12,height=12, units='cm',res=180)
p = ggplot(data=av2[[2]], mapping=aes(x=factor(Year),y=Variance,fill=WB))+
  geom_col(width=0.5,position='stack')+
  geom_hline(yintercept=c(av5[[2]][1,3]), size=1,col="grey52")+
  labs(x = 'Year', y='Imbalance') +
  #scale_x_continuous(breaks=seq(1985,2018, by=5))+
  theme(
	  text = element_text(size=18),
	  panel.background = element_rect(fill = "transparent",colour = 'black'), 
...
for (i in 4:6){
png(filename=paste0(home, '/Results/',modelname,'/Balance/ImbDev',names(cv)[i],'.png'),
		  width=13,height=12, units='cm',res=180)
...
png(filename=paste0(home, '/Results/',modelname,'/Balance/ImbalanceP.png'),
		  width=13,height=12, units='cm',res=180)
