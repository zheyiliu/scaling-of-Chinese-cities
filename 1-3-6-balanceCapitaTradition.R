...
dir.create(paste0("C:/Sync/CoolGirl/Fhe/Results/",modelname,"/capitaTradition"),showWarnings = F)
setwd(paste0("C:/Sync/CoolGirl/Fhe/Results/",modelname,"/capitaTradition"))
file.remove(dir())
...
for (yi in 1:length(dflist)){
	...
	cordfall = na.omit(cordfall)
	popyc = aggregate(list(cordfall$xindex,cordfall$yindex), by=list(cordfall$year, cordfall$yIndex), sum)
	colnames(popyc) = c('year','yIndex','N','Y')
	popyc$YN = popyc$Y / popyc$N
	popyc$YN.s = (popyc$YN - min(popyc$YN))/(max(popyc$YN)-min(popyc$YN))
	
	save(cordfall, file=paste(home,'/Results/',modelname,'/capitaTradition/cordfall_',ydfname,'.Rdata',sep=''))
    write.csv(cordfall, file=paste(home,'/Results/', modelname,'/capitaTradition/cordfall_',ydfname,'.csv',sep=''))
    
	cordfally = rbind(cordfally, cordfall)
	popycy = rbind(popycy, popyc)
	
	chooseyeard = as.character(c(1985,1993,2002,2010,2018))
	fordensd = fordens[chooseyeard]
	fordensd = lapply(fordensd, na.omit)
	thed = lapply(fordensd, density, adj=1)
	...
	png(filename=paste0(home,'/Results/',modelname,'/capitaTradition/Fig_',ydfname,'_capita.png'),
		  width=14,height=14, units='cm',res=180)
	plot(x=c(),y=c(),xlim=range(unlist(fordensd)), ylim=range(drange),type="n",
	...
	legend("topright", legend=chooseyeard, col=cols[2:(2+length(thed))],lty=1,lwd=2)             
	dev.off()
}
  
save(cordfally, file=paste(home,'/Results/',modelname,'/capitaTradition/cordfally.Rdata',sep=''))
write.csv(cordfally, file=paste(home,'/Results/', modelname,'/capitaTradition/cordfally.csv',sep=''))
save(popycy, file=paste(home,'/Results/',modelname,'/capitaTradition/popycy.Rdata',sep=''))
write.csv(popycy, file=paste(home,'/Results/', modelname,'/capitaTradition/popycy.csv',sep=''))
