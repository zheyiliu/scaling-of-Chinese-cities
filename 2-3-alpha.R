...
Alpha = function(dfname, rangeStat, yeari, scale, denfun){
  ...
  if(length(cs)<50){
    ...
    if (dfname=='POP'){
      png(file=paste0('C:/Sync/CoolGirl/Fhe/Results/3powerlaw/alpha/',dfname,yeari,denfun,'.png'),
          width = 480, height = 480, units = "px")
      plot(m_m, ann=F)
      lines(m_m,col=2,lw=3,lty=3)
      abline(v=m_m$xmin, col=rgb(0, 0, 255, 60, maxColorValue=255), lw=3)
      l1 = paste0(dfname,' (',yeari,')')
      l2 = paste0('α = ',round(m_m$pars,3), ' (Pks=', round(bs_p$p,3), ')')
      l3 = paste0("Nmin = ", round(m_m$xmin,2))
      ...
      png(file=paste0('C:/Sync/CoolGirl/Fhe/Results/3powerlaw/tau/',dfname,yeari,denfun,'.png'),
          width = 480, height = 480, units = "px")
      plot(m_m, ann=F)
      lines(m_m,col=2,lw=3,lty=3)
      abline(v=m_m$xmin, col=rgb(0, 0, 255, 60, maxColorValue=255), lw=3)
    ...
    alla = data.frame(yIndex=paste0(dfname,rangeStat), Alpha=m_m$pars, KS_Pvalue=bs_p$p, Observation=length(cs), year=yeari)
  }
  alla
}

AlphaAll = function(dfname0, rangeStat0, yeari0, scale0, denfun0){
  all0 = data.frame()
  for (yearii in yeari0){
    ee = Alpha(dfname0, rangeStat0, yearii, scale0, denfun0)
    all0 = rbind(all0, ee)
  }
