...
for (year in 1985:2017){
  ...
  for (f in fileyearraw){
    ...
    needtitle=''
    if (year<=2017){ #year<=1992
      needtitle = dat[1,1]
      needtitle = gsub('表|\\d','', needtitle)
      needtitle = gsub('[[:punct:]]|[\n *)]|[a-z]|[A-Z]','',needtitle)
      needtitle = paste0(needtitle, '_')
    }
    danwei = ''
    ...
    dat = subset(dat, !grepl(pattern='合计|总计|超大|特大|大城市|中等|小城市|东部|中部|西部|全部|沿海|特区|492|72|14|4|\\d个|人口|东都|中都', dat[,1]) & !(dat[,1] %in% c(prov,provfull)))
    dat[,1] = gsub('城市名称|城市名|地市|程市|城巾|城、市', '城市', dat[,1])
    ### 对正常格式的表格清理指标名（行名）
    findcities = which(dat[,1] %in% c(citylist,citylistshi))
    if (length(findcities) < 20){
      notdata = c(notdata, f)
      next
    }
    if (!strangeformat){
      ...
      cityline = grep('\\b城市\\b|\\b城市名称\\b|\\b城市名\\b|\\b地市\\b|\\b程市\\b|\\b城巾\\b|\\b城、市\\b', dat[,1])
      citycol = grep('\\b城市\\b|\\b城市名称\\b|\\b城市名\\b|\\b地市\\b|\\b程市\\b|\\b城巾\\b|\\b城、市\\b', dat)
...
df = data.frame()
save(df, file='C:/Sync/CoolGirl/Fhe/ecosocialDATA/SuperData/SuperData.Rdata')
...
###输入假的数据库
  ...
  for (f in fileall){
    ...
    load(file='C:/Sync/CoolGirl/Fhe/ecosocialDATA/SuperData/SuperData.Rdata')
    df = rbind(df,sql)
    ###输入假的数据库 done!
