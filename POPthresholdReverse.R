...
for (yeari in 1985:2017){
	...
	a = na.omit(POPd$value)
	#piseq = seq(min(POPd$value), quantile(POPd$value, probs=0.9), 1)
	piseq = seq(max(a), quantile(a, probs=0.1), length.out = 100)
	for (p in piseq){
...
for (yeari in 1985:2017){
  ...
  a = na.omit(POPc$value)
  if (length(a) != 0){
    #piseq = seq(min(a), quantile(a, probs=0.9), length.out=100)
    piseq = seq(min(a), quantile(a, probs=0.1), length.out = 100)
    for (p in piseq){
