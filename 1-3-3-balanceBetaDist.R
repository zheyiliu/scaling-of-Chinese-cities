...
write.csv(sustain, file=paste0(home,'/Results/', modelname,'/Balance/imbalanceIndex.csv'))
...
png(filename=paste0(home, '/Results/',modelname,'/Balance/imbalanceIndex.png'),
		  width=20,height=10, units='cm',res=180)
...
png(filename=paste0(home, '/Results/',modelname,'/Balance/balanceIndex3.png'),
...
png(filename=paste0(home, '/Results/',modelname,'/Balance/balanceIndex4.png'),
		  width=16,height=15, units='cm',res=180)
...
meansqT = c(meansqT, av[1,3])
meansqR = c(meansqR, av[2,3])
}
aovdf = data.frame(
		Year=1984:2017,
		WB=c(rep('Between dimensions',2018-1984),rep('Within dimensions',2018-1984)),
		Variance=c(meansqT,meansqR)
)

png(filename=paste0(home, '/Results/',modelname,'/Balance/balanceIndex5.png'),
		  width=25,height=15, units='cm',res=180)
p = ggplot(data=aovdf, mapping=aes(x=Year,y=Variance,fill=WB))+
  geom_col(width=0.5,position='stack')+
  labs(x = 'Year(1984-2017)', y='the Greedy-Imbalance Index') +
  scale_x_continuous(breaks=seq(1985,2018, by=5))+
  theme(
	  text = element_text(size=18),
	  panel.background = element_rect(fill = "transparent",colour = 'black'), 
...
suml$Betadiff = suml$Beta-suml$theory

meansqT = c()
meansqR = c()
for (yeari in 1985:2018) {
sumly = suml[suml$year == yeari,]
av = anova(lm(Betadiff~type, data=sumly))
meansqT = c(meansqT, av[1,3])
meansqR = c(meansqR, av[2,3])
}
aovdf = data.frame(
...
png(filename=paste0(home, '/Results/',modelname,'/Balance/balanceIndex6.png'),
		  width=25,height=15, units='cm',res=180)
p = ggplot(data=aovdf, mapping=aes(x=Year,y=Variance,fill=WB))+
  geom_col(width=0.5,position='stack')+
  labs(x = 'Year(1984-2017)', y='the Ideal-Imbalance Index') +
  scale_x_continuous(breaks=seq(1985,2018, by=5))+
  theme(
	  text = element_text(size=18),
	  panel.background = element_rect(fill = "transparent",colour = 'black'), 
