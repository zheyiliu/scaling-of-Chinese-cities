...
for (yeari in 1985:2017){
	...
	piseq = seq(floor(min(POPd$value)), floor(quantile(POPd$value, probs=0.9)), 1)
	for (p in piseq){
...
for (yeari in 1993:2017){
  rangeStat = rangeStatList[1]
  POPc = subset(POPdensity, grepl(rangeStat,POPdensity$index) & POPdensity$year==yeari)
  POPd = subset(POP, grepl(rangeStat,POP$index) & POP$year==yeari)
  #hist(POPc$value, breaks=80, xlab='Population density in Districts(person per km^2)', main='')
  a = na.omit(POPc$value) # upper quantile = 167.5
  if (length(a) != 0){
    piseq = seq(min(a), quantile(a, probs=0.9), length.out=100)
    for (p in piseq){
      cityset = subset(POPc, POPc$value >= p)$city
      POPset = subset(POPd, POPd$city %in% cityset)
      Beta = vector()
      Intercept = vector()
      Pvalue = vector()
      BetaLower = vector()
...
save(sumlmHorizontal, file=paste('C:/Sync/CoolGirl/Fhe/Results/PDthreshold/sumlmHorizontal_',rangeStatList[2],'.Rdata',sep=''))
write.csv(sumlmHorizontal, file=paste('C:/Sync/CoolGirl/Fhe/Results/PDthreshold/sumlmHorizontal_',rangeStatList[2],'.csv',sep=''))
...
setwd(paste0('C:/Sync/CoolGirl/Fhe/Results/PDthreshold'))
...
for (yname in unique(sumlmHorizontal$yIndex)){
	for (yeari in unique(sumlmHorizontal$year)){
		...
		labs(x = 'Population Density Threshold', y='β', title=paste0(gsub(rangeStatList[1], '', yname),'.',rangeStatList[2])) +
